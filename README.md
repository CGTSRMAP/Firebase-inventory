...
## Admin console with login (Firebase)

`manage.html` is a password-protected console for adding, editing, and deleting
equipment and contacts in the browser — no more hand-editing CSV files. It uses
Firebase Authentication for login and Firestore to store records.

- **First-time setup:** follow `FIREBASE_SETUP.md` step by step (create the project,
  enable Email/Password sign-in, create your admin user, create the database, and
  **publish `firestore.rules`** — that file is what protects your data).
- **Connecting the project:** your Firebase web config lives in `data/firebase.json`.
  These web keys are safe to keep in a public repo; access is controlled by the rules.
- **Loading existing data:** sign in to `manage.html` and click **Import CSV** once to
  pull your current `data/csv/equipment.csv` and `data/csv/contacts.csv` into Firebase.

Note: the public finder (`index.html`) and `admin.html` still read the CSV files, so
edits made in the console won't appear there until those pages are pointed at Firebase
(a small change to their data-loading step).
